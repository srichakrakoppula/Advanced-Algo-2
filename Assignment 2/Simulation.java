...
public class Simulation {
    ...
    public static void main(String[] args) {
       ...
       /* int numNodes = 100;
        double p_graph = .1; // parameter for random graph: prob. that an edge will exist
        double p_malicious = .01; // prob. that a node will be set to be malicious
        double p_txDistribution = 10; // probability of assigning an initial transaction to each node
        int numRounds = 54; // number of simulation rounds your nodes will run for
*/
    	int numNodes = 1000;
        ...
        int numTx = 50;
        HashMap<Integer, String> validTxIds = new HashMap<Integer, String>();
        Random random = new Random();
        for (int i = 0; i < numTx; i++) {
            int r = random.nextInt();
            String encodedMsg = Encoder.encodeToBase58(String.valueOf(r));
            validTxIds.put(r, encodedMsg);
        }
        ...
        for (int i = 0; i < numNodes; i++) {
            ...
            for(Integer txID : validTxIds.keySet()) {
                if (Math.random() < p_txDistribution) // p_txDistribution is .01, .05, or .10.
                    pendingTransactions.add(new Transaction(txID, validTxIds.get(txID)));
            }
        ...
        for (int round = 0; round < numRounds; round++) { // numRounds is either 10 or 20
            ...
            for (int i = 0; i < numNodes; i++) {
                ...
                for (Transaction tx : proposals) {
                    if (!validTxIds.keySet().contains(tx.id))
        ...
        // print results
        int count = 0;
        Set<Transaction> transactionsInConsensus = new HashSet<>();
        int flag = 0;
        for (int i = 0; i < numNodes; i++) {
...
//
        ...
        System.out.println("Number of nodes that reached consensus = " + String.valueOf(count));
        System.out.println();
        System.out.println("Transactions in the consensus : ");
        for (Transaction tx : transactionsInConsensus)
            System.out.println(tx.addr);
    }
